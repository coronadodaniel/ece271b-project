...
from conv3d_model import ResNet,BasicBlock
...
def main():
    ...
    if not os.path.exists('log'):
        os.makedirs('log')
    model_path = 'model/model_Conv3d'+'.pt'
    
    logger = logging.getLogger('netlog')
    logger.setLevel(logging.INFO)
    ch = logging.FileHandler('log/logfile_Conv3d'+'.log')
    ...
    model = ResNet(BasicBlock,[2,2,2,2],arg.batchSize,arg.windowSize)
    #print(model)
    if arg.useGPU_f:
        if len(arg.gpu_num)>1:
    ...
    for epoch in range(epochs):
        ...
        for batchIdx,(windowBatch,labelBatch) in enumerate(trainLoader.batches(arg.batchSize)):
            windowBatch = windowBatch.permute(0,2,1,3,4)
            optimizer.zero_grad()
            #loss=0.0
            if arg.useGPU_f:
